...
```{r}
# Transforma os dados para o formato longo
vendas_medias_categoria <- dados_vendas_longo %>%
  group_by(categoria) %>% 
  summarise(vendas_medias = mean(valor_vendas)) %>% 
  arrange(desc(vendas_medias))
    # Visualiza o resultado
vendas_medias_categoria
```
...
```{r}
valor_total_vendas <- dados_vendas_longo %>% 
  groupby(categoria) %>% 
  summarise(
    vendas_totais = sum(vendas_mensais),
    media_mensal = mean(vendas_mensais),
    valor_minimo = min(vendas_mensais),
    valor_maximo = max(vendas_mensais)
  ) %>% 
